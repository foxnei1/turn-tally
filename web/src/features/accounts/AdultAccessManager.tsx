...
import { adultCommand, type AdultAccounts, type AdultAccount } from './adultApi'
...
export function AdultAccessManager({ client, onBack }: { client: SupabaseClient; onBack: () => void }) {
  const [data, setData] = useState<AdultAccounts | null>(null)
  const [generation, setGeneration] = useState(0)
  const [code, setCode] = useState('')
  const [review, setReview] = useState<{ code: string; email: string } | null>(null)
  const [personId, setPersonId] = useState('')
  const [additional, setAdditional] = useState(false)
  const [revoking, setRevoking] = useState<AdultAccount | null>(null)
  ...
  useEffect(() => {
    let active = true
    setData(null)
    void adultCommand<AdultAccounts>(client,'list').then(next => { if (active) setData(next) }).catch(error => { if (active) setError(error.message) })
    return () => { active = false }
  }, [client, generation])
  ...
  async function command(operation: string, payload: Record<string, unknown>, success: string) {
    ...
    try {
      await adultCommand(client,operation,payload)
  ...
  return <main className="mx-auto w-full max-w-xl space-y-5 px-5 py-8">
    ...
    <h1 className="text-3xl font-semibold">Adult access</h1>
    <p>Link an adult sign-in to an existing family member. The account uses that person's current permissions.</p>
    ...
    {!data && !error ? <p role="status">Loading adult access…</p> : null}
    {data ? <>
      <section className="space-y-4 rounded-2xl border border-stone-200 bg-white p-5">
        <h2 className="text-xl font-semibold">Link adult account</h2>
        {!review ? <form onSubmit={async event => {
          event.preventDefault(); setBusy(true); setError(null); setMessage(null)
          try {
            const next = await adultCommand<{ email: string }>(client,'lookup',{ code })
            const latest = await adultCommand<AdultAccounts>(client,'list')
          ...
          <p>Compare code <strong>{review.code}</strong> with the adult's screen before approving.</p>
          ...
          <button className={button} disabled={busy || !person || (!!duplicates && !additional)} onClick={() => void command('approve',{ code:review.code,person_id:personId,confirm_additional:additional },'Adult access approved.')}>Approve access</button>
      ...
      {data.accounts.map(account => <article key={account.user_id} className="space-y-3 rounded-2xl border border-stone-200 bg-white p-5">
        ...
        {revoking?.user_id === account.user_id ? <div className="space-y-3">
          ...
          <button className={button} disabled={busy} onClick={() => void command('revoke',{ user_id:account.user_id },'Adult access revoked.')}>Confirm revocation</button>
