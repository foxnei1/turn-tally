...
import { parentalCommand, ParentalAccessError, type ParentalRequest } from './parentalApi'

const button = 'rounded-xl bg-emerald-800 px-4 py-3 font-semibold text-white disabled:opacity-50'
export function ParentalLinking({ client, onLinked }: { client: SupabaseClient; onLinked: () => void }) {
  const [request, setRequest] = useState<ParentalRequest | null>(null)
  ...
  useEffect(() => {
    ...
    const poll = window.setInterval(async () => {
      ...
      checking = true
      try {
        const next = await parentalCommand<{ state: string }>(client, 'status', { id:request.id })
        if (!active) return
        setError(null)
        if (next.state === 'approved') { setState('approved'); onLinked() }
        else if (next.state !== 'pending') setState(next.state)
      } catch (error) {
        if (active && !(error instanceof ParentalAccessError && error.status === 429)) {
          setError(error instanceof Error ? error.message : 'Could not check this code.')
          if (error instanceof ParentalAccessError && [403,410].includes(error.status)) setState('unavailable')
        }
  ...
  return <main className="mx-auto w-full max-w-md space-y-5 px-5 py-10">
    <h1 className="text-3xl font-semibold">Link your parental account</h1>
    <p>Show a linking code to a parent. They will choose your family profile and approve your access.</p>
    {request && state === 'pending' ? <>
      <p aria-label="Parental linking code" className="rounded-xl bg-white p-5 text-center font-mono text-3xl font-semibold">{request.code}</p>
      ...
      <button className={button} disabled={busy} onClick={async () => {
        ...
        try {
          const next = await parentalCommand<{ state: string }>(client,'cancel',{ id:request.id })
          setState(next.state)
      ...
      <button className={button} disabled={busy || state === 'approved'} onClick={async () => {
        setBusy(true); setError(null)
        try { const next = await parentalCommand<ParentalRequest>(client,'start'); setRequest(next); setState('pending') }
