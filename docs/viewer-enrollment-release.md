...
- Revoking the shared test enrollment through the service coordinator blocked reads using its existing token and cleared its displayed family data on the access check. The other viewer remained usable. Both browsers then passed disconnect-before-parental-sign-in and cleared their Auth sessions; both test enrollments and memberships are now revoked.
...
- The viewer checks access every 60 seconds while visible and on focus, visibility return, and reconnect. Failed checks discard the displayed family; refreshing explicitly reloads after connectivity returns. Switching to parental sign-in first revokes viewer membership and clears its session. No durable family cache or editing offline is included.
