...
`turntally_load` returns only the caller's provisioned household. Membership revocation, inactive people, and current roles are checked from stored data. `viewer_only` caps a provisioned identity at viewer access even if the person's role is later promoted. The new viewer-device migration uses that cap for personal and shared browser enrollments; it is implemented locally and not deployed.
...
6. Provision other adult Auth accounts and link each to the same household and its existing person ID through an administrative SQL session. Do not infer roles from emails or roster position. Adult account linkage remains operator-managed. The local Devices screen manages viewer enrollments only.
