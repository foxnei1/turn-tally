...
The release was built from viewer checkpoint `d5c54fa` in the isolated `turntally-startup-hotfix` worktree, with only `useTurnTally.ts` and its regression test changed. All 173 release web tests, lint, types, and build passed; the main working tree also passed all 185 tests including deferred recovery. Published HTML, JavaScript, and CSS matched the tested release; SPA fallback, Auth origin access, disabled signup/anonymous access, and unauthenticated household rejection passed. Recovery remained undeployed. No database migration or direct household mutation was needed. The regression test reproduces database key ordering and verifies new assignments are recorded once across repeated loads. The owner confirmed that the signed-in app loaded correctly afterward.
